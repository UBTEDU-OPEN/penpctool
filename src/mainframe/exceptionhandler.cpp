...
StackTracer::StackTracer(void)
    :m_dwExceptionCode(0)
{
    LOG(INFO) << "StackTracer::StackTracer begin";
    // Get machine type
    m_dwMachineType = 0;
    size_t Count = 256;
    ...
    LOG(INFO) << "StackTracer::StackTracer end";
}
...
{
    LOG(INFO) << "StackTracer::HandleException begin";
    ...
    // ...
    SymCleanup(hProcess);
    LOG(INFO) << "StackTracer::HandleException end";
    return(EXCEPTION_EXECUTE_HANDLER);
}
...
{
    LOG(INFO) << "StackTracer::TraceCallStack begin";
    ...
    LOG(INFO) << "StackTracer::TraceCallStack end " << m_vecCallStack.size();
}

#endif

ExceptionHandler::ExceptionHandler(QObject *parent) : QObject(parent)
{
